<template>
	<div id="travelApproval">
		<div class="list">
			<div class="listHeader">
				<p @click="chooseFilterType(item.type)" :class="{'active':item.type==activeFilterType}" v-for="item in filterType">{{item.name}}</p>
			</div>
			<div class="warp" v-for="item in data">
				<div class="shenpi">
					...
					<div class="status">
						...
						<router-link :to="{ path: `/home/task/travelapprovaldetail?id=${item.id}`}">
				...
				<table>
					<tr v-for="route in item.routes">
						...
						<td class="handle">
							<div>
								...
								<p v-if="route.routeState==2" @click="toOrder(route,'route')" class="dingNum">{{route.orderNumber}}</p>
							</div>							
						</td>
						<td class="date">
					...
					<tr v-for="hotel in item.hotels">
						...
						<td class="handle">
							...
							<p v-if="hotel.hotelState==2" @click="toOrder(hotel,'hotel')" class="dingNum">{{hotel.orderNumber}}</p>
						</td>
						<td class="date">
