			...
			<div class="warp" v-for="item in data">
				<div class="shenpi">
					...
					<div class="status">
						...
						<router-link :to="{ path: `/home/task/travelbookingdetail?id=${item.id}`}">
				...
				<table>
					<tr v-for="route in item.routes">
						...
						<td class="handle">							
							<div v-if="route.routeState==0"><Button @click="booking(item,route,'route')" class="preDing">预定</Button></div>							
							<div v-if="route.routeState!=0">
								<p>{{route.routeState|childState}}</p>
								<p v-if='route.routeState==2' class="dingNum" @click="toOrder(route,'route')">{{route.orderNumber}}12</p>
							</div>
						</td>
						<td class="date">
					...
					<tr v-for="hotel in item.hotels">
						...
						<td class="handle">
							<div v-if="hotel.hotelState==0"><Button @click="booking(item, hotel,'hotel')" class="preDing">预定</Button></div>							
							<div v-if="hotel.hotelState!=0">
								<p>{{hotel.hotelState|childState}}</p>
								<p v-if='hotel.hotelState==2' class="dingNum" @click="toOrder(hotel,'hotel')">{{hotel.hotelNumber}}</p>
							</div>
						</td>
						<td class="date">
